...
def data_table_tab():
    ...
    # Debug: Show channel count for troubleshooting
    st.write(f"DEBUG: Found {len(st.session_state.channel_colors)} channels: {list(st.session_state.channel_colors.keys())}")

    # Statistics (moved up to match other tabs)
    if len(df) > 0:
