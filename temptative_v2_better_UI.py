...
def main():
    ...
    /* Column highlighting */
    [data-testid="stDataFrame"] tbody tr td:nth-child(1n+1) {
        position: relative;
    }
    ...
    # Tab layout with improved icons
...
def data_table_tab():
    ...
    st.markdown("""
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 15px; margin: 10px 0; text-align: center;">
        <h2 style="margin: 0; font-size: 2em;">📊 Data Insights Dashboard</h2>
        <p style="margin: 5px 0; font-size: 1.2em;">Explore your marketing analytics with advanced filtering and visualization</p>
    </div>
    ...
    if len(df) > 0:
        ...
        # Quick statistics with enhanced visuals
        st.subheader("⚡ Key Metrics Overview")

        numeric_cols = df.select_dtypes(include=[np.number]).columns

        if len(numeric_cols) > 0:
            # Default to a key metric
            default_metric_idx = 0
            if 'impressions_cost' in numeric_cols:
                default_metric_idx = list(numeric_cols).index('impressions_cost')
            elif 'ROAS' in numeric_cols:
                default_metric_idx = list(numeric_cols).index('ROAS')

            selected_metric = st.selectbox(
                "🔍 Choose metric to analyze:",
                options=numeric_cols,
                index=default_metric_idx,
                key="data_workspace_overview_metric"
            )

            # Enhanced metrics display
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                metric_value = df[selected_metric].mean()
                st.metric(
                    f"**📊 Average** {selected_metric}",
                    f"{metric_value:.2f}",
                    help=f"Mean value across all {len(df)} rows"
                )
            with col2:
                metric_value = df[selected_metric].max()
                st.metric(
                    f"**📈 Maximum** {selected_metric}",
                    f"{metric_value:.2f}",
                    help=f"Highest value in dataset"
                )
            with col3:
                metric_value = df[selected_metric].min()
                st.metric(
                    f"**📉 Minimum** {selected_metric}",
                    f"{metric_value:.2f}",
                    help=f"Lowest value in dataset"
                )
            with col4:
                metric_value = df[selected_metric].std()
                st.metric(
                    f"**📊 Std Deviation** {selected_metric}",
                    f"{metric_value:.2f}",
                    help=f"Variation in the data"
                )
    ...
    with col1:
        ...
        sort_by = st.selectbox("Sort by column (descending)", options=sort_options, key="data_workspace_sort_by")
    ...
    if sort_by != 'None':
        ascending = False  # Always descending for data workspace
